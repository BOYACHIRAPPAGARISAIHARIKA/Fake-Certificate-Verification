...
# Function to generate heat map with pixel analysis (ELA for images, score-based for others)
def generate_heat_map(file_bytes, file_type, score):
    if file_type in ['image/jpeg', 'image/png']:
        # For images, use ELA to detect tampering
        image = Image.open(BytesIO(file_bytes))
        ela_diff = perform_ela(image)
        ela_array = np.array(ela_diff.convert('L'))  # Convert to grayscale array
        
        # Create heatmap: High ELA differences (tampering) in red, low in green
        fig, ax = plt.subplots()
        cax = ax.imshow(ela_array, cmap='RdYlGn_r', interpolation='nearest')  # Reverse for red=high (danger), green=low (safe)
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(data, cmap='RdYlGn_r', interpolation='nearest')  # Red for low score (danger), green for high (safe)
    ...
    if uploaded_file is not None:
        try:
            ...
            if file_type not in ["application/pdf", "image/jpeg", "image/png"]:
                ...
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.subheader("🗺️ Pixel Analysis Heat Map")
                heat_map_fig = generate_heat_map(file_bytes, file_type, score)
